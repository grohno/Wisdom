...
<html>
	...
	<div id="loading"></div>
		...
		<article>
			<h1 class="balloon">質問を投稿します</h1>
			<%
				@SuppressWarnings("unchecked")
				ArrayList<Category> categories= (ArrayList<Category>) request.getAttribute("categories");
			%>
			<form action="<%=Constant.PATH_WISDOM_CNTL %>" method="post">
				<input type="hidden" name="<%=Constant.PARAM_ACTION%>" value="<%=Constant.ACTION_QUE_CREATE%>">

				<table class="post">
					<tr>
						<th>タイトル</th>
						<td><input type="text" name="<%=Constant.PARAM_QUE_TITLE%>" required></td>
					</tr>
					<tr>
						<th>本文</th>
						<td><textarea name="<%=Constant.PARAM_QUE_CONTENT%>" required></textarea></td>
					</tr>
					<tr>
						<th>カテゴリー</th>
